...
class EmailSender:
    def __init__(self, config_path: str = "config.json", env_path: str = ".env", dry_run: bool = False, selected_email: Optional[str] = None) -> None:
        ...
        self.email_user = selected_email
        self.email_password = email_options[self.email_user]
        ...
        self.default_body: str = self.config.get('default_body', 'Confira nossa oferta exclusiva!')
    ...
    def render_template(self, context: Dict) -> str:
        try:
            ...
            if not os.path.exists(template_path):
                logging.error(f"Template '{self.template_file}' não encontrado no diretório 'templates'")
            ...
            try:
                ...
                css_content = ""
                logging.warning("Arquivo de estilo 'style.css' não encontrado.")

            context["inline_css"] = css_content
    ...
    def send_bulk_emails(self, contacts: List[Dict[str, str]], attachments: Optional[List[str]] = None) -> None:
        ...
        # Validação antecipada dos anexos
        if attachments:
            valid_attachments = [file for file in attachments if os.path.exists(file)]
            invalid_attachments = [file for file in attachments if not os.path.exists(file)]
            for file in invalid_attachments:
                logging.warning(f"Arquivo de anexo não encontrado e será ignorado: {file}")
